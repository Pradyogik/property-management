...
import { getNewProperties } from "../../services/api";
import {
  ChevronLeft,
  ChevronRight,
  Download,
...
import { NewPropertyDetailsModal } from "../NewPropertyDetailsModal";
import NewEditPropertyForm from "../NewEditPropertyDetailsModal";
// import { EditPropertyDetailsModal } from "../EditPropertyDetailsModal";


export default function TableV4() {
...
const freeHoldAmt = getFreeHoldAmt();
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(
      (record) =>
        (categoryFilter === "all" || String(record?.category).toLowerCase() === categoryFilter.toLowerCase()) &&
